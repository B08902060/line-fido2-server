 ...
 */
 ...
 public class UserKeyServiceImpl implements UserKeyService {
     ...
     private String hashCredentialId(String rawCredentialId) {
         try {
             MessageDigest digest = MessageDigest.getInstance("SHA-256");
             byte[] hashBytes = digest.digest(rawCredentialId.getBytes());
             return Base64.getUrlEncoder().withoutPadding().encodeToString(hashBytes);  // 轉換為 Base64 字串
         } catch (NoSuchAlgorithmException e) {
             throw new RuntimeException("Failed to hash credentialId", e);
         }
     }
     ...
     public UserKey createUser(UserKey user) {
         if (user.getRk() != null && user.getRk()) {
             user.setCredentialId(hashCredentialId(user.getCredentialId()));
         }
     ...
     public boolean containsCredential(String rpId, String credentialId) {
         ...
         UserKeyEntity userKeyEntity2 = userKeyRepository.findByRpEntityIdAndCredentialId(rpId, hashCredentialId(credentialId));
         
         return (userKeyEntity != null) || (userKeyEntity2 != null);
     }
     ...
     public UserKey getWithCredentialId(String rpId, String credentialId) {
         ...
         if (userKeyEntity == null){
             userKeyEntity = userKeyRepository.findByRpEntityIdAndCredentialId(rpId, hashCredentialId(credentialId));
         }
     ...
     public void deleteWithCredentialId(String rpId, String credentialId) {
         UserKeyEntity userKeyEntity = userKeyRepository
                 .findByRpEntityIdAndCredentialId(rpId, credentialId);
         if (userKeyEntity == null) {
             credentialId = hashCredentialId(credentialId);
             userKeyEntity = userKeyRepository.findByRpEntityIdAndCredentialId(rpId, credentialId);
             if (userKeyEntity == null) {
